...
<?php
	...
	if(isset($_POST['create'])){
		if(!isset($_POST['f_name']) || empty($_POST['f_name'])){
			...
			$db->select('user','mobile',null,"mobile = '{$params["mobile"]}'",null,null);
			$exists = $db->getResult();
			...
			if(!empty($exist)){
				...
				echo json_encode(array('error'=>'Mobile Number Already Exists.')); exit;
			}
			else{
	...
	if(isset($_POST['login'])){
		if(!isset($_POST['username']) || empty($_POST['username'])){
			...
			echo json_encode(array('error'=>'Password Foeld is Empty.')); exit;
		}else{
			$db = new Database();
